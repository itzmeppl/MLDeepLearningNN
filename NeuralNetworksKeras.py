...
print("\nThe softmax action function was use for the output layer because it converts a vector of values to a probability distribution") #5 points
...
print("\nThe size of the first hidden layer is 300. None means the batch size or a placeholder.\
The total number of parameters of the first hidden layer is 235500, which refers to the # of bias plus (number of inputs * number of neurons) which are the learnable prarameters") #8 points
model.summary()
...
print("\nAn epoch acts as a hyperparameter in machine learning. It is a complete pass through the entire training dataset, each time it runs, the sample updates the model’s parameters. The number of epochs controls how many times the model will iterate over the dataset and with each iteration, model's learning is refined.") #6 points
